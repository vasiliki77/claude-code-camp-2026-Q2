...
def state_dir(args):
    ...
    d = os.path.join(d, f"{args.host}-{args.port}-{args.user}")
...
class Session:
    def __init__(self, host, port, user, password, logpath, new_sex="M", new_class="W"):
        self.host, self.port = host, port
        self.user, self.password = user, password
        self.logpath = logpath
        # Only consulted if the server treats `user` as a brand-new character
        # (see the creation-wizard branch in login()); ignored otherwise.
        self.new_sex, self.new_class = new_sex, new_class
        self.sock = None
    ...
    def login(self, timeout=45):
        ...
        A name the server has never seen forks into a creation wizard instead
        of a password prompt: confirm spelling -> set password -> retype it
        -> sex -> class, which then rejoins the normal flow at the same
        "PRESS RETURN" gate and account menu. `new_sex`/`new_class` answer
        that fork; they are never consulted for an existing character.
        """
        deadline = time.time() + timeout
        sent = set()
        while time.time() < deadline:
            ...
            if re.search(r"(by what name|what is your name|name:)", tail, re.I) and "name" not in sent:
                ...
                sent.add("name")
            elif re.search(r"did i get that right", tail, re.I) and "confirm_name" not in sent:
                self.write("y")
                sent.add("confirm_name")
            elif re.search(r"give me a password", tail, re.I) and "newpass" not in sent:
                self.write(self.password)
                sent.add("newpass")
            elif re.search(r"retype password", tail, re.I) and "retype" not in sent:
                self.write(self.password)
                sent.add("retype")
            elif re.search(r"what is your sex", tail, re.I) and "sex" not in sent:
                self.write(self.new_sex)
                sent.add("sex")
            elif re.search(r"select a class", tail, re.I) and "class" not in sent:
                self.write(self.new_class)
                sent.add("class")
...
def serve(args, sd):
    ...
    sess = Session(args.host, args.port, args.user, args.password, p["log"],
                    new_sex=args.sex, new_class=args.char_class)
    try:
        sess.connect()
    except OSError as e:
...
def cmd_start(args, sd):
    ...
    subprocess.Popen(
        [sys.executable, os.path.abspath(__file__),
         ...
         "--sex", args.sex, "--class", args.char_class,
         "_serve"],
...
def cmd_reset(args, sd):
    ...
    # Reconnect as the same character this invocation was already for.
    reset_args = argparse.Namespace(**vars(args))
    reset_args.timeout = 60
    return cmd_start(reset_args, sd)
...
def main():
    ...
    ap.add_argument("--sex", choices=["M", "F"], default="M",
                     help="only used if --user names a brand-new character")
    ap.add_argument("--class", dest="char_class", choices=["C", "T", "W", "M"], default="W",
                     help="only used if --user names a brand-new character")
