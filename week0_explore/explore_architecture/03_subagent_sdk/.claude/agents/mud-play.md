...
`.claude/agents/scripts/mud.py` solves this by running a small background
...
```bash
python3 .claude/agents/scripts/mud.py setup --wimpy 8
```
...
python3 .claude/agents/scripts/mud.py send north east "kill fido"    # runs in order
python3 .claude/agents/scripts/mud.py setup --wimpy 8                # idempotent game settings
python3 .claude/agents/scripts/mud.py read --wait 5                  # collect what arrived on its own
python3 .claude/agents/scripts/mud.py expect "is dead" --timeout 30  # block until text appears
python3 .claude/agents/scripts/mud.py status                         # alive? where? what vitals?
python3 .claude/agents/scripts/mud.py log -n 80                      # replay the transcript
python3 .claude/agents/scripts/mud.py reset                          # quit and reconnect (recover from stuck states)
python3 .claude/agents/scripts/mud.py stop                           # quit out cleanly
```
...
```bash
python3 .claude/agents/scripts/mud.py send "consider fido"           # check the matchup first
python3 .claude/agents/scripts/mud.py send "kill fido"               # opening round
python3 .claude/agents/scripts/mud.py expect "is dead|You are dead|flee" --timeout 40
```
...
```bash
python3 .claude/agents/scripts/mud.py reset
```
...
python3 .claude/agents/scripts/update_memory.py

# Memory files, alongside the scripts:
# - .claude/agents/data/player.md   ← character level, HP, location, goals
# - .claude/agents/data/world.md    ← map, mobs, quest info, target locations
```
...
1. Start session, read `.claude/agents/data/player.md` to see current level
...
5. Once at level 7, search `.claude/agents/data/world.md` for Minotaur location
