...
class Logger:
    ...
    # Only the REPL numbers turns; the one-shot run() path never calls this.
    # See _turn_count below for why session_end cannot rely on it alone.
    def turn(self, *, n):
        self._turns_started = max(self._turns_started, int(n))
    ...
    # The larger of "turns the caller numbered" and "turns that ended", because
    # neither is right on its own:
    #
    #   - Only Repl calls turn(); the one-shot run() path never does, so the
    #     first version of session_end reported turns=0 for every non-REPL
    #     session — including the 05-08 mapping run, which was 80 iterations of
    #     one turn and said zero.
    #   - Counting turn_end alone undercounts a session interrupted mid-turn,
    #     where the turn started and never finished.
    #
    # Taking the max reports started-turns for the REPL, 1 for a one-shot run,
    # and includes the in-flight turn when a session is cut short.
    def _turn_count(self):
        return max(self._turns_started, self._turns_ended)
