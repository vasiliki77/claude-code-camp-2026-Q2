...
class Logger:
    ...
    # Ruby also checks the symbol form of each key; a JSON-decoded dict in
    # Python only ever has string keys. Ruby's rescue sits at method level, so a
    # non-numeric value yields nil rather than trying the next key — matched
    # here by returning on the failed conversion instead of continuing.
