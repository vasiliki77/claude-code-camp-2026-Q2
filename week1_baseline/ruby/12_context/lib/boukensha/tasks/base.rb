module Boukensha
  module Tasks
    class Base
      ...
      # The second per-turn circuit breaker: cumulative input+output tokens
      # spent this turn, independent of how many iterations that took. 0
      # disables it.
      def self.max_turn_tokens(settings)
        integer_setting(settings, :max_turn_tokens, DEFAULT_MAX_TURN_TOKENS)
      end

      # Fraction of the model's context window at which the agent compacts
      # before its next API call. Lives beside the other limits rather than in a
      # global `agent:` block so a task's ceilings are all configured in one
      # place — a second task will want its own.
      def self.compaction_threshold(settings)
        value = fetch(settings, :compaction_threshold)
        return DEFAULT_COMPACTION_THRESHOLD if value.nil?
