...
> The carry-forward was a genuine two-way merge, not a copy: the step-12 snapshot was branched before the MCP and tasks work, while adding real work of its own. `docs/plans/step_deltas/12_context` records what collided and how each conflict was settled. **After this merge step 12 is the canonical line** — week 2 branches from it, and no further step gets patched forward from an older snapshot.

- **Accurate token tracking** — `Context` now tracks `context_window` (the model's max input capacity, from `Boukensha::Models.context_window(model)`) separately from `current_tokens` (input tokens of the most recent API response — window pressure) and `turn_tokens` (cumulative spend this turn), fixing an earlier display that conflated the output-token budget with the context window and let a cumulative session sum grow unbounded past `/clear`.
- **`Boukensha::Usage`** — one shim that knows where each provider hides its token counts (`usage.input_tokens`, `usage.prompt_tokens`, `usageMetadata.promptTokenCount`, top-level `prompt_eval_count`). Agent, Logger and the TUI all read through it. Reading Anthropic's key names directly leaves the gauge at 0% and the compaction trigger dead on every other backend, silently.
- **`Boukensha::Models`** — a model → capability lookup folded from every backend's own `MODELS` table at load time, so `Context` can be sized before a backend is constructed. Derived rather than hand-written because the hand-written copy had already drifted (it claimed `claude-sonnet-4-6` was 200k while the backend said 1M). Unknown models fall back to a conservative default rather than assuming a huge window.
- **Colour-coded context indicator** — grey under 70% used, yellow 70–84%, red (with a `⚠`) at 85%+, shown in both the progress and status lines. The status bar carries the percentage only; the absolute used/max pair stays on the progress line, since the bar also carries version, model, tool count, MUD route and clock.
- **Auto-compaction** — at the start of each turn, if usage crosses the task's `compaction_threshold` (default 0.85), `Context#compact_messages!` drops the oldest ~40% of messages (keeping at least 2) before the next API call. **The drop point is snapped forward to a plain user turn** — dropping by count alone orphans a `tool_result` whose `tool_use` went with it, which Anthropic answers with a 400, and tool pairs are most of a MUD session's history. `/compact` triggers it manually from the REPL or TUI. Emits a `Logger#compaction` event that the TUI renders as a notice in the conversation view.
- **A second circuit breaker** — `Agent` now stops a turn on whichever trips first: `max_iterations` (tool-call count) or the new `max_turn_tokens` (cumulative input+output tokens spent this turn). Both are configured per task in `settings.yaml` (`tasks.<name>.*`), beside `max_output_tokens` and `compaction_threshold` — one place to change a ceiling, and a second task can have its own. `0` disables a ceiling.
- **A test suite** — the first step with gates for its own behaviour rather than only the MCP layer: token maths and the compaction invariant, the same counts from all four provider usage shapes, both circuit breakers, and the REPL commands. No API key, no MUD, no network.
